...
# 导入tkinter的全部模块, 并重命名为tk, tk.Label  tk.Button
# tkinter用于创建软件界面，提供窗口、按钮、标签、文本框等控件
...
class El6312A:
    ...
    def dynamic(self, channel, load_max, time1):
        ...
        self.el.write('CURR:DYN:FALL 2.5')
        self.el.write('CURR:DYN:RISE 2.5')
...
class El63600:
    ...
    def dynamic(self, channel, load_max, time1):
        ...
        self.el.write('CURR:DYN:L1 %.2f' % (0.8 * load_max))  # 设置负载的上下电流值
        self.el.write('CURR:DYN:L2 %.2f' % (0.2 * load_max))
        self.el.write('CURR:DYN:T1 %.2fms' % time1)           # 设置电平持续时间
        self.el.write('CURR:DYN:T2 %.2fms' % time1)
        self.el.write('CURR:DYN:FALL 2.5')                    # 设置切换速率
        self.el.write('CURR:DYN:RISE 2.5')
        self.el.write('CURR:DYN:REP 0')         # 设置重复次数 无限循环
...
def go():  # 从 Excel 文件中提取数据并更新相关全局变量
    # 处理事件，*args表示可变参数
    global volt, ld_max, freq, CHANNEL, ocp_spec, temp, xls, vin, Iin, file_path           # 全局变量声明
    ...
    CHANNEL= int(EnValue5.get())  # 从 EnValue2 获取通道号并转换为整数
...
def savepic():
    picture_cell_mapping = {
        "T0-1": "N1",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T1-1": "M18",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T1-2": "U18",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T1-3": "AC18",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T2-1": "F40",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T2-2": "R40",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T2-3": "AD40",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T3-1": "F67",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T3-2": "F86",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T3-3": "R67",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T3-4": "R86",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T4-1": "F108",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T4-2": "F127",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T4-3": "R108",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T-4": "R127",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T5-1": "F149",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T5-2": "F168",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T5-3": "R149",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T5-4": "R168",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T6-1": "F190",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T6-2": "F209",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T6-3": "R190",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T6-4": "R209",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T9-1": "F281",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T9-2": "F299",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T9-3": "F281",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T9-4": "F299",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格
        "T10-1": "F321",  # 例如 "T2-1" 的图片应该插入到 Excel 的 "M18" 单元格

        # 你可以继续添加更多的对应关系
    }


    osc_dir = os.path.normpath(f'C:/POL/{temp.strip()}/{entry.get().strip()}')

    osc.readfile(osc_dir)
    print(f"[成功] 已从示波器读取图片文件：{osc_dir}")


    # 构造路径
    # 去除 entry.get() 和 temp 中的空格
    for pic_key, cell in picture_cell_mapping.items():
        picture_dir = os.path.normpath(f"{pic_path}/POL Test Pictures/{temp.strip()}/{entry.get().strip()}")
        picture_path = os.path.normpath(f"{picture_dir}/{pic_key}.PNG")
        ...
        # try:
        #     osc.makeDir('C:\\POL')
        #     osc.makeDir(f'C:\\POL\\{temp}')
        #     osc.makeDir(f'C:\\POL\\{temp}\\{entry.get()}')
        ...
        #
        # # 读取示波器文件
        # saved_image_path = os.path.normpath(f'{osc_dir}/{pic_key}.PNG')
        # if pic_key not in picture_cell_mapping:
        #     print(f"[提示] 文件 {pic_key}.PNG 不在字典中，已跳过")
        #     continue  # 跳过不在字典中的文件
        # try:
...
def tl2_channel_set():
    ...
    # 设置通道1的相关参数
    osc.chanset('CH1', -3, volt, '20.0000E+06', 5e-2)
...
def main_window():
    global root, EnValue1, EnValue2, EnValue3, EnValue4, EnValue5, entry
    root = tk.Tk()  # 创建主窗口实例
    ...
    EnValue1.set('P2V5_BMC_STBY')
    entry = tk.Entry(root, show=None, width=20, textvariable=EnValue1)
    entry.grid(row=4, column=1, columnspan=2)

    # 新增：通道选择输入框
    tk.Label(root, text='通道选择:').grid(row=6, column=0, sticky=tk.E)
    EnValue5 = tk.StringVar()  # 使用 EnValue5 存储通道号
    EnValue5.set('1')  # 默认值
    tk.Entry(root, show=None, width=10, textvariable=EnValue5).grid(row=6, column=1)
    tk.Label(root, text='（填入1/3/5/7/9等）').grid(row=6, column=2, sticky=tk.W)
    ...
    tk.Button(root, text="图片导入", command=savepic).place(x=70, y=420, width=80, height=30)
...
def test0():
    ...
    print(f"获取TD值: {tdc}")
    el.static(CHANNEL, 'MAX', tdc)  # 选择相应的通道
    el.state('ON')
    ...
    while i <= 3:
        if type == 'ALL':
            ...
            if i == 1:
                ...
                print('正在设置水平位置')
                el.dynamic(CHANNEL, ld_max, 2.5)
                print('正在设置电子负载模式')
                time.sleep(2)
                print("模式 1 设置完成")
            elif i == 2:
                set_horizontal_mode(5e-4, 5e-4, 55, 2e8)
                el.dynamic(CHANNEL, ld_max, 0.5)
                time.sleep(2)
                print("模式 2 设置完成")
            elif i == 3:
                set_horizontal_mode(50e-6, 50e-6, 55, 2e8)
                el.dynamic(CHANNEL, ld_max, 0.05)
                time.sleep(2)
                print("模式 3 设置完成")
            ...
            el.dynamic(CHANNEL, ld_max, dynamic_param)
            time.sleep(2)
            print(f"{type} 模式设置完成")
        ...
        el.state('OFF')
        el.static(CHANNEL, 'MAX', 0)  # 选择相应通道
        print("负载关闭")
        time.sleep(1)
...
def test3(type):
    ...
    test_modes = {
        ...
        'upmax': {
            'horizontal_params': (8e-3, 1e-2, 60, 2e8),  # 水平模式参数
            'trigger': 'RISE',  # 触发边沿
            'load': (CHANNEL, 'MIN', ld_max, True),  # 负载设置参数
            'index': 3  # 模式索引
        },
        'downmax': {
            'horizontal_params': (8e-3, 1e-2, 60, 2e8),  # 水平模式参数
            'trigger': 'FALL',  # 触发边沿
            'load': (CHANNEL, 'MIN', ld_max, False),  # 负载设置参数
            'index': 4  # 模式索引
        }
    }
    ...
    while i <= 4:
        ...
        if type == 'ALL':
            if i == 1:
                ...
                set_horizontal_mode(8e-3, 8e-3, 60, 2e8)
                set_load(CHANNEL, 'MIN', ld_max, lvp_on=True)
                osc.trigger('NORMAL', 'CH1', 'RISE',volt / 2)
            elif i == 4:
                set_horizontal_mode(8e-3, 8e-3, 60, 2e8)
                set_load(CHANNEL, 'MIN', ld_max, lvp_on=False)
                osc.trigger('NORMAL', 'CH1', 'FALL',volt / 2)
        else:
            i = mode_params['index']
            print(f"执行单项测试：{type} 模式开始")
...
def test4(type):
    ...
    test_modes = {
        ...
        'OM': {
            ...
            'process': 'ch1max',
            'xls_position': (111, 15),
            'load': (CHANNEL, 'MIN', ld_max, True),  # Set load for OM
            'index': 3  # 添加索引值
        },
        'UM': {
            'measure': 'FALL',
            ...
            'process': 'ch1min',
            'xls_position': (130, 3),
            'load': (CHANNEL, 'MIN', ld_max, False),  # Set load for UM
            'index': 4  # 添加索引值
        }
    }
    if type != 'ALL':
...
def test5(type):
    ...
    test_modes = {
        ...
        'sh': {
            ...
            'process': 'ch4freq',
            'xls_position': (152, 15),
            'load': (CHANNEL, 'MIN', ld_max),  # No load setup for UN
            'cursor': None,
            'index': 2  # 添加索引值
        },
        'jl': {
        ...
        'jh': {
            ...
            'process': 'poswidth',
            'xls_position': (175, 15),
            'load': (CHANNEL, 'MIN', ld_max),  # Set load for UM
            'cursor': True,
            'index': 4  # 添加索引值
        }
    }
    ...
    if table_type == '表格1':
        while i <= 4:
            ...
            if type == 'ALL':
                ...
                if i == 1:
                    ...
                    measure5('FRE')
                    el.static(CHANNEL, 'MAX', ld_max-1)
                    el.state('ON')
                    set_osc()
                    connect = messagebox.askquestion(title='电路接switch',
                                                     message='请进行电路连接，连接成功请点击是确认存图，失败请点击否')
        ...
        while i <= 4:
            ...
            if i == 2 or i == 4:
                el.static(CHANNEL, 'MAX', ld_max)  # 拉载处理
                el.state('ON')  # 开启电源
                print("拉载开始，负载已开启")
...
def test6(type):
    ...
    test_modes = {
        ...
        'hsmax': {
            'load': (CHANNEL, 'MAX', ld_max),  # No load setup for UN
            'index': 2  # 添加索引值
        },
        'lsno': {
            'load': None,  # Set load for OM
        ...
        'lsmax': {
            'load': (CHANNEL, 'MAX', ld_max),  # Set load for UM
            'index': 4  # 添加索引值
        }
    }
    if type !='ALL':
...
def test8():
    ...
    for test_current in test_currents:
        ...
        # 设置电流
        el.static(CHANNEL, 'MAX', test_current)       #13579 大负载机通道
        el.state('ON')
        time.sleep(5)
